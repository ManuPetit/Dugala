<?php
	...
	//		ajout d'une nouvelle photo pour la page d'accueil
	
	
	require('../inclusion/config.inc.php');
	//verifier que l'utilisateur est logge
	redirect_invalid_user();
	$page_title = "Ajouter une nouvelle photo pour la page d'accueil";
	$_SESSION['menuid'] = 2;
	include('../inclusion/admintete.php');
	require('../../dugala.inc.php');
	include('../inclusion/admin_form_function.inc.php');
	$pto_errors=array();
	echo '<h2>Ajouter une nouvelle photo pour la page d\'accueil</h2>
	<p>Cette page vous permet de télécharger sur votre site une nouvelle photo pour l\'utiliser comme photo sur votre page d\'accueil.</p>';
	if (($_SERVER['REQUEST_METHOD'] == 'POST') && (isset($_POST['psub']))) {
			//validation des entrées
			if (trim($_POST['nomfr']) != '') {
				if (preg_match('/^[^<>"]{4,150}$/u',stripslashes($_POST['nomfr']))) {
					$nfr = escape_data($_POST['nomfr']);
				} else {
					$pto_errors['nomfr'] = "<br />Certains caract&egrave;res ne sont pas accept&eacute;s. Veuillez corriger.";
				}
			} else {
				$nfr = 'Photo du restaurant';
			}
			if (trim($_POST['nomgb']) == '') {
				//on prend la traduction française
				if (isset($nfr)) {
					if ($nfr == 'Photo du restaurant') {
						$ngb = 'Picture from the restaurant';
					} else {
						$ngb = $nfr;
						$_POST['nomgb'] = $nfr;
					}
			...
			} else if (preg_match('/^[^<>"]{4,150}$/u',stripslashes($_POST['nomgb']))) {
			...
			if (empty($pto_errors)) {
				...
				$v = $_POST['visible'];
				if ($v==1){
					//on met tous les autres images à zéro
					$sql = 'UPDATE photoopen SET IsShowing = 0';
					$r = @mysqli_query($dbc, $sql);
				}
				//requete
				$q = "INSERT INTO photoopen (fichier, desc_fr, desc_gb, IsShowing, datecreer) VALUES ('";
				$q .= $name . "','" . $nfr . "','" . $ngb . "'," . $v . ", NOW())";
...
?>
<fieldset><legend>D&eacute;tails de la nouvelle photo d'accueil : </legend>
<form action="photoacc_ajou.php"  method="post" accept-charset="utf-8" style="padding-left:100px" enctype="multipart/form-data">
<input type="hidden" name="MAX_FILE_SIZE" value="5242880" />
<p><label for="nomfr"><strong>L&eacute;gende de la photo en fran&ccedil;ais (facultatif) :</strong></label><br />
<?php create_form_input('nomfr','text',$pto_errors,70,150); ?>
</p>
<p><label for="nomgb"><strong>L&eacute;gende de la photo en anglais (facultatif) :</strong></label><br />
<?php create_form_input('nomgb','text',$pto_errors,70,150); ?>
...
<input type="hidden" name="psub" value="TRUE" />
</form>
</fieldset>
<?php
	include('../inclusion/adminpied.php');
